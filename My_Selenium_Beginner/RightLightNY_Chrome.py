...
class ChromeSearch(unittest.TestCase):
    ...
    def test_search(self):
        ...
        driver.get("https://rightlightny.com/")
        print("Right Light of NY Url has ", requests.get("https://rightlightny.com/").status_code, " as status Code")
        if not "Right Light of NY" in driver.title:
            raise Exception("Unable to load Right Light of NY page!")

        # Check that an element is present on the DOM of a page and visible.
        try:
            WebDriverWait(driver, 2).until(EC.visibility_of_element_located((By.XPATH, "(//span[@data-ux='Element']["
                                                                                       "contains(.,'Right Light  "
                                                                                       "NY')])[4]")))
            sleep(3)
            print('First RLNY page is ready!')
            print(driver.title)

        except TimeoutException:
            print("Loading took too much time!")
            assert 'About Us' in driver.page_source
            print("All images presented on the page")

            wait.until(EC.visibility_of_element_located((By.XPATH, '//img[contains(@alt,"image57")]')))
            wait.until(EC.visibility_of_element_located((By.XPATH, "//div[@data-ux='Block'][contains(@id,"
                                                                   "'parallax3144')][contains(.,'Right Light NY(917) "
                                                                   "597-6289')]")))
